...
export interface PluginDatabaseAccess {
  ...
  update?: (table: string, data: Record<string, unknown>, where: string | object, parameters?: unknown[]) => Promise<DatabaseResult>;
  ...
  private createDatabaseAccessInterface(pluginName: string, config: PluginContextConfig): PluginDatabaseAccess {
    return {
      ...
      update: async (
        ...
        return this.databaseAccessService.update(pluginName, config.databaseAccess, table, data, where as string, parameters);
      },
