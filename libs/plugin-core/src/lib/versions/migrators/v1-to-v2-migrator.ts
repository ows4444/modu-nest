...
} from '../v1/plugin-interfaces-v1';
import { 
  PluginManifest, 
  GuardEntry, 
  PluginMetadata 
} from '../../manifest/plugin-manifest.types';
import { 
  MigrationRules,
  PluginAPIVersion,
  createVersionedInterface
} from '../interface-versions';
...
export class PluginManifestMigrationRules implements MigrationRules<LegacyPluginManifest, PluginManifest> {
  apply(
    from: LegacyPluginManifest, 
    fromVersion: PluginAPIVersion, 
  ...
  apply(
    ...
    const v2Guard: Omit<GuardEntry, keyof VersionedInterface> = {
      ...
      __migrated: {
        ...
        migratedAt: new Date().toISOString(),
        warnings: [
          'Guard migrated from v1.0 to v2.0',
          'Consider adding security restrictions and dependencies',
        ],
      },
    };
