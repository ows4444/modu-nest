...
import {
  PluginGuardDependencyOptimizer,
  OptimizationResult,
  DependencyAnalysis,
} from './plugin-guard-dependency-optimizer';
...
export class PluginGuardManager {
   ...
   */
  async resolveGuardsForPluginWithOptimization(
    pluginName: string,
    requestedGuards: string[]
  ): Promise<GuardResolutionResult> {
    return await this.resolutionMutex.runExclusive(async () => {
      this.logger.debug(
        `Resolving guards with optimization for plugin '${pluginName}': [${requestedGuards.join(', ')}]`
      );
      ...
      const orderToUse =
        optimizationResult.optimizedOrder.length > 0 ? optimizationResult.optimizedOrder : requestedGuards;
