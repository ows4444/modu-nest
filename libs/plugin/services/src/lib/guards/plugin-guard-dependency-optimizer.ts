...
import { GuardEntry, LocalGuardEntry } from '@plugin/core';
...
export class PluginGuardDependencyOptimizer {
  ...
  suggestGuardComposition(guardNodes: GuardNode[]): {
    ...
    const optimizedComplexity =
      originalComplexity - compositeGuards.reduce((sum, comp) => sum + (comp.components.length - 1), 0);
  ...
  private findCriticalPath(guardNodes: GuardNode[]): string[] {
    const maxLevelNode = guardNodes.reduce((max, node) => (node.level > max.level ? node : max), guardNodes[0]);
  ...
  private generateOptimizationSuggestions(
    ...
    if (deeplyNestedGuards.length > 0) {
      suggestions.push(
        `Consider flattening deeply nested guards (level > 3): ${deeplyNestedGuards.map((n) => n.name).join(', ')}`
      );
    }
    ...
    if (heavyDependencyGuards.length > 0) {
      suggestions.push(
        `Consider breaking down guards with many dependencies: ${heavyDependencyGuards.map((n) => n.name).join(', ')}`
      );
    }
