...
import { PluginVersionUtils, CompatibilityResult } from '@plugin/validation';
import { PluginValidationError, PluginManifestError, PluginManifest } from '@plugin/core';
...
export class PluginCompatibilityChecker {
  ...
  private static checkAPIVersionCompatibility(pluginVersion: string): {
    isCompatible: boolean;
    warnings: string[];
  } {
    const warnings: string[] = [];
