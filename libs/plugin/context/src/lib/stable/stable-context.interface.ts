...
import { VersionedInterface, PluginAPIVersion } from '@plugin/core';
...
export class StableContextError extends Error {
  constructor(message: string, public readonly code: string, public readonly context?: object) {
    super(message);
...
export class StableLimitError extends StableContextError {
  constructor(limitType: string, current: number, maximum: number) {
    super(`${limitType} limit exceeded: ${current}/${maximum}`, 'LIMIT_EXCEEDED', { limitType, current, maximum });
