...
import { BaseFrameworkError, ErrorCategory, ValidationError, NotFoundError, ConflictError } from '@shared/utils';
...
export class PluginRegistryConnectionError extends PluginError {
  ...
  constructor(registryUrl: string, originalError?: Error) {
    super(`Failed to connect to plugin registry at ${registryUrl}`, {
      registryUrl,
      originalError,
    });
  }
...
export class PluginInstallationError extends PluginError {
  ...
  constructor(pluginName: string, reason: string) {
    super(`Failed to install plugin '${pluginName}': ${reason}`, {
      pluginName,
      reason,
    });
  }
