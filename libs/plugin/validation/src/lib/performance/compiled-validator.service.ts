...
export class CompiledValidatorService {
  ...
  private async getCompiledValidator(validationType: 'full' | 'essential' | 'trusted'): Promise<CompiledValidator> {
  ...
  private async compileValidator(validationType: 'full' | 'essential' | 'trusted'): Promise<CompiledValidator> {
    const startTime = performance.now();
