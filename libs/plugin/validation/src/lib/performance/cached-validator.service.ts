...
export class CachedValidatorService {
  ...
  private async performValidation(
    ...
    if (options.skipTrustedValidation && manifest.security?.trustLevel === 'internal') {
   ...
   */
  private performEssentialValidation(manifest: Partial<PluginManifest>, failFast?: boolean): PluginValidationResult {
    const errors: string[] = [];
   ...
   */
  private performFullValidation(manifest: Partial<PluginManifest>, failFast?: boolean): PluginValidationResult {
  ...
  private cleanupCache(): void {
    ...
    if (cleanedCount > 0) {
      this.logger.debug(
        `Cleaned up ${cleanedCount} expired cache entries, reclaimed ${this.formatBytes(reclaimedMemory)}`
      );
    }
  }
  ...
  private updateMetrics(validationTime: number): void {
    ...
    this.metrics.averageValidationTime = this.metrics.validationTime / this.metrics.totalValidations;
  }
}
