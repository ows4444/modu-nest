...
export class PerformanceValidatorService implements OnModuleInit {
  ...
  async validateManifests(
    manifests: Array<{ manifest: Partial<PluginManifest>, options?: PerformanceValidationOptions }>,
    ...
    this.logger.log(`Batch validated ${manifests.length} manifests in ${totalTime.toFixed(2)}ms (${batchStrategy} strategy)`);
  ...
  private selectBatchStrategy(
    ...
    const trustedCount = manifests.filter(({ manifest }) => 
  ...
  private async batchValidateHybrid(
    ...
    for (const item of manifests) {
      if (item.manifest.security?.trustLevel === 'internal' || 
          item.options?.essential || 
          globalOptions.essential) {
  ...
  private updatePerformanceMetrics(validationTime: number): void {
    ...
    const totalTime = (this.performanceMetrics.averageValidationTime * (this.performanceMetrics.totalValidations - 1)) + validationTime;
