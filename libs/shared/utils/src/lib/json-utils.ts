...
export function parseJsonOrThrow(json: string, errorMessage?: string): any {
  try {
    ...
    throw new Error(errorMessage || `Failed to parse JSON: ${(originalError as Error).message}`);
  }
}
 ...
 */
export function flattenJson(obj: Record<string, any>, prefix = '', separator = '.'): Record<string, any> {
