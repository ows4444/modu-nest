...
@UseGuards(PluginRateLimitingGuard, PluginTrustEnforcementGuard)
@ApiBearerAuth()
export class PluginTrustController {
  ...
  @ApiResponse({ status: 404, description: 'Plugin not found' })
  async assignTrustLevel(
    @Param('name') name: string,
    @Body() assignmentDto: AssignTrustLevelDto
  ) {
    const assignment = {
      ...assignmentDto,
      pluginName: name,
  ...
  async validateCapability(
    @Param('name') name: string,
    @Body() body: { capability: string; version?: string }
  ) {
    const result = await this.pluginRegistryService.validatePluginCapability(
      name, 
      body.capability, 
      body.version
    );
    ...
    return {
      ...
      reason: result.allowed 
        ? 'Capability allowed by trust level policy' 
        : 'Capability denied by trust level policy'
    };
  }
  ...
  @ApiResponse({ status: 404, description: 'Plugin not found' })
  async validateTrustPolicy(
    @Param('name') name: string,
    @Body() body: { manifest: any; version?: string }
  ) {
    const result = await this.pluginRegistryService.validatePluginTrustPolicy(
      name,
      body.manifest,
      body.version
    );
