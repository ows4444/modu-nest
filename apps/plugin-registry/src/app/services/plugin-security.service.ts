...
export class PluginSecurityService {
  ...
  validateFileSize(pluginBuffer: Buffer, pluginName?: string): void {
    if (pluginBuffer.length > this.SECURITY_CONFIG.MAX_FILE_SIZE) {
      ...
      this.errorMetrics.recordError(error, { pluginName, operation: 'file-size-validation' });
  ...
  async validatePluginSecurityWithCache(pluginBuffer: Buffer, checksum: string): Promise<void> {
    ...
    if (cachedResult) {
      ...
      if (!cachedResult.isValid) {
        ...
        this.errorMetrics.recordError(error, { operation: 'cached-security-validation' });
        handlePluginError(error, { operation: 'validatePluginSecurityWithCache' });
      }
  ...
  private async validatePluginSecurity(pluginBuffer: Buffer): Promise<void> {
    try {
      ...
      if (unsafeResults.length > 0) {
        ...
        this.errorMetrics.recordError(error, { operation: 'import-validation' });
      ...
      this.errorMetrics.recordError(securityError, { operation: 'security-validation' });
  ...
  private scanForUnsafeImports(content: string): string[] {
    ...
    try {
      ...
      this.errorMetrics.recordError(securityError, { operation: 'import-scan' });
