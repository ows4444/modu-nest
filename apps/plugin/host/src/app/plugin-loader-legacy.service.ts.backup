...
export class PluginLoaderService implements PluginLoaderContext, IPluginEventSubscriber {
  ...
  private async performPluginDiscovery(): Promise<{
  ...
  private async discoverPluginsWithEnhancedErrorHandling(): Promise<PluginDiscoveryResult> {
    ...
    try {
      ...
      const pluginDirs = await fs.promises.readdir(pluginsPath, {
        withFileTypes: true,
      });
    ...
    try {
      if (guardEntry.scope === 'local') {
        ...
        validationErrors.push(
          `Invalid guard scope '${(guardEntry as any).scope}' for guard '${(guardEntry as any).name}'`
        );
      }
    } finally {
