...
export class PluginVersionController {
  ...
  async rollbackVersion(@Param('name') name: string, @Body() rollbackDto: RollbackVersionDto) {
    const options = {
    ...
    return await this.pluginRegistryService.rollbackPluginVersion(name, rollbackDto.targetVersion, options);
  }
  ...
  async checkVersionCompatibility(@Param('name') name: string, @Body() compatibilityDto: VersionCompatibilityDto) {
  ...
  @ApiResponse({ status: 404, description: 'Plugin or version not found' })
  async downloadPluginVersion(@Param('name') name: string, @Param('version') version: string) {
