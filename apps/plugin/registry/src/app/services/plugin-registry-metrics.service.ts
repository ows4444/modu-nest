...
export class PluginRegistryMetricsService {
  ...
  recordValidation(validationType: string, duration: number, success: boolean, metadata?: Record<string, any>): void {
    ...
    if (validationType in this.metrics.performance.validationTimes) {
      this.metrics.performance.validationTimes[
        validationType as keyof typeof this.metrics.performance.validationTimes
  ...
  private getOperationMetrics(operation: string): OperationMetrics {
    switch (operation) {
      case 'upload':
        return this.metrics.uploads;
      case 'download':
        return this.metrics.downloads;
      case 'validation':
        return this.metrics.validations;
      case 'search':
        return this.metrics.searches;
      default:
        return this.metrics.uploads; // fallback
    }
  }
  ...
  private addToHistory(metric: string, value: number): void {
    ...
    this.metricsHistory.set(
      metric,
      history.filter((point) => point.timestamp.getTime() > cutoffTime)
    );
  }
  ...
  private cleanupOldMetrics(): void {
    ...
    this.operationHistory = this.operationHistory.filter((op) => op.timestamp.getTime() > cutoffTime);
  ...
  getOperationalStats(): {
    ...
    const topOperations = [
      {
        operation: 'uploads',
        count: this.metrics.uploads.total,
        averageTime: this.metrics.uploads.averageResponseTime,
      },
      {
        operation: 'downloads',
        count: this.metrics.downloads.total,
        averageTime: this.metrics.downloads.averageResponseTime,
      },
      {
        operation: 'validations',
        count: this.metrics.validations.total,
        averageTime: this.metrics.validations.averageResponseTime,
      },
      {
        operation: 'searches',
        count: this.metrics.searches.total,
        averageTime: this.metrics.searches.averageResponseTime,
      },
