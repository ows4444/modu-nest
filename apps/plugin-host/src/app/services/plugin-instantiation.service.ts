...
export class PluginInstantiationService {
  ...
  async instantiatePlugins(
    ...
    loadOrder: string[]
  ): Promise<PluginInstantiationResult> {
    ...
    for (const pluginName of loadOrder) {
      ...
      try {
        ...
        const instantiationError = this.createInstantiationError(
          pluginName,
          error as Error,
          'MODULE_LOADING'
        );
    ...
    this.logger.log(
      `Plugin instantiation completed: ${successful.length} successful, ${failed.length} failed`
    );
  ...
  private async loadPluginModule(pluginPath: string, manifest: PluginManifest): Promise<Type<any>> {
    ...
    try {
      ...
      if (!ModuleClass) {
        throw new Error(`Module class not found in ${pluginMainPath}. Expected: ${(manifest as any).moduleClass || 'default'}`);
      }
